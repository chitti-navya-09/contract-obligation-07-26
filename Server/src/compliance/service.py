...
from src.entities.compliance import ComplianceRecord
...
def get_dashboard_summary(db: Session) -> Dict[str, Any]:
    ...
    avg_score = db.query(func.avg(ComplianceRecord.score)).scalar() or 84.0
    compliant_contracts = db.query(ComplianceRecord).filter(ComplianceRecord.status == "Compliant").count()
    critical_violations = db.query(ComplianceRecord).filter(ComplianceRecord.status == "Non-Compliant").count()
    pending_audits = db.query(ComplianceRecord).filter(ComplianceRecord.status == "Under Review").count()
...
def get_compliance_trend(db: Session) -> List[Dict[str, Any]]:
    ...
    trend_query = db.query(
        func.to_char(ComplianceRecord.last_audit, "MM").label("month_num"),
        func.to_char(ComplianceRecord.last_audit, "Mon").label("month_name"),
        func.avg(ComplianceRecord.score).label("score")
...
def get_risk_distribution(db: Session) -> Dict[str, int]:
    ...
    high_count = db.query(ComplianceRecord).filter(ComplianceRecord.risk == "High").count()
    medium_count = db.query(ComplianceRecord).filter(ComplianceRecord.risk == "Medium").count()
    low_count = db.query(ComplianceRecord).filter(ComplianceRecord.risk == "Low").count()
...
def get_per_contract_compliance(
    ...
    if risk:
        query = query.filter(ComplianceRecord.risk == risk)
    if category:
        query = query.filter(ComplianceRecord.category == category)
    if search_query:
        query = query.filter(
            (ComplianceRecord.requirement.ilike(f"%{search_query}%")) |
            (ComplianceRecord.entity.ilike(f"%{search_query}%"))
        )

    total = query.count()
    records = query.order_by(ComplianceRecord.id.asc()).offset(skip).limit(limit).all()
...
def get_all_compliance_records(db: Session, skip: int = 0, limit: int = 100) -> List[ComplianceRecord]:
    """
    Retrieves all compliance records.
    """
    return db.query(ComplianceRecord).order_by(ComplianceRecord.id.asc()).offset(skip).limit(limit).all()

def get_compliance_record_by_id(db: Session, record_id: int) -> Optional[ComplianceRecord]:
    """
    Retrieves a single compliance record by ID.
    """
    return db.query(ComplianceRecord).filter(ComplianceRecord.id == record_id).first()

def create_compliance_record(db: Session, record_data: Dict[str, Any]) -> ComplianceRecord:
    """
    Creates a new compliance record.
    """
    # Map camelCase to snake_case for SQLAlchemy instantiation
    mapping = {
    ...
    for field in ["last_audit"]:
        ...
        if isinstance(val, str) and val:
            try:
                cleaned_data[field] = datetime.strptime(val, "%Y-%m-%d").date()
            except ValueError:
                cleaned_data[field] = None

    db_record = ComplianceRecord(**cleaned_data)
    db.add(db_record)
    db.commit()
    db.refresh(db_record)
    return db_record

def update_compliance_record(db: Session, record_id: int, record_data: Dict[str, Any]) -> Optional[ComplianceRecord]:
    """
    Updates an existing compliance record.
    """
    db_record = db.query(ComplianceRecord).filter(ComplianceRecord.id == record_id).first()
    if not db_record:
        return None
